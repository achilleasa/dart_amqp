...
main({bool enableLogger = true}) {
  ...
  group("Exchanges:", () {
    ...
    tearDown(() {
      return client.close().then((_) => client2.close());
    });
    ...
    test("check if unknown exchange exists", () {
      client
          ...
          .then((Channel channel) =>
              channel.exchange("foo123", ExchangeType.DIRECT, passive: true))
    ...
    test("declare exchange", () {
      client
          .channel()
          .then((Channel channel) =>
              channel.exchange("ex_test_1", ExchangeType.DIRECT))
    ...
    test("declare exchange and bind private queue consumer", () {
      client
          ...
          .then((Exchange exchange) =>
              exchange.bindPrivateQueueConsumer(["test"]))
    ...
    test("declare exchange and bind multiple routing keys", () {
      client
          ...
          .then((Channel channel) =>
              channel.exchange("ex_test_1", ExchangeType.DIRECT))
          .then((Exchange exchange) =>
              exchange.bindPrivateQueueConsumer(["test", "foo", "bar"]))
          .then(expectAsync1((Consumer consumer) {
        expect(consumer.channel, const TypeMatcher<Channel>());
        expect(consumer.queue, const TypeMatcher<Queue>());
        expect(consumer.tag, isNotEmpty);
    ...
    test("publish to FANOUT exchange without a routing key", () {
      client
          .channel()
          .then((Channel channel) =>
              channel.exchange("ex_test_2", ExchangeType.FANOUT))
    ...
    group("exceptions", () {
      test("missing exchange name", () {
        return client.channel().then((Channel channel) {
          expect(
      ...
      test("missing routing key for non-fanout exchange publish", () {
        return client
            .channel()
            .then((Channel channel) =>
                channel.exchange("test", ExchangeType.DIRECT))
            .then((Exchange exchange) {
          expect(
      ...
      test("bind to non-FANOUT exchange without specifying routing key", () {
        ...
        client
            ...
            .then((Channel channel) =>
                channel.exchange("test", ExchangeType.DIRECT))
            .then((Exchange exchange) {
          exchange.channel.privateQueue().then((Queue queue) {
            expect(
                () => queue.bind(exchange, ""),
                throwsA((ex) =>
                    ex is ArgumentError &&
